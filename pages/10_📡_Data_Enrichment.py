...
# 1. Coverage heatmap
# ──────────────────────────────────────────────────────────────────────
st.subheader("1 · Coverage heatmap")
...
if matrix.empty or go is None:
    st.info("No ENRICH_FLAG column available yet.")
else:
...
for name, cols in SOURCE_COLUMNS.items():
    ...
    for c in present:
        ...
        if mask.any():
            ts = pd.to_datetime(df.loc[mask, ts_col], utc=True, errors="coerce")
            ts = ts.dropna()
            if not ts.empty:
                most_recent = ts.max()
...
# ──────────────────────────────────────────────────────────────────────
st.subheader("3 · Manual backfill trigger")
...
with col3:
    ...
    selected = st.multiselect(
        "Sources", options=source_choices, default=source_choices,
    )
...
if not gh_token:
    ...
    st.button("Trigger backfill", disabled=True)
else:
    trigger = st.button("Trigger backfill", type="primary")
    if trigger:
        if requests is None:
            ...
            try:
                ...
                if resp.status_code in (200, 204):
                    ...
                    st.error(f"GitHub API returned {resp.status_code}: {resp.text[:200]}")
            except Exception as exc:  # noqa: BLE001
...
if summary["n_flagged"].sum() > 0:
    st.caption(
        "Individual flagged rows (first 50):"
    )
