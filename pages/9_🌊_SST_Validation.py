                ...
                if pd.notna(_phase):
                    if abs(_phase) >= 6:
                        st.error(
                            ...
                            "timezone. The fetcher now auto-detects each "
                            "row's IANA zone from its own (lat, lon) via "
                            "`timezonefinder`, so the fix is to:\n\n"
                            "1. **Install** `timezonefinder` on the cron "
                            "runner (already in `requirements.txt`).\n"
                            "2. **Clear** the `ENRICH_FLAG` column for "
                            "the affected rows so the next cron run "
                            "re-fetches Open-Meteo at the correct UTC "
                            "hour.\n"
                            "3. If rows were written by an Apps Script "
                            "whose sheet is in a fixed non-UTC zone "
                            "AND the buoy's GPS is in a DIFFERENT zone, "
                            "override the GPS auto-detection via the "
                            "`SHEETS_DISPLAY_TZ` secret (e.g. set it to "
                            "the spreadsheet's configured zone)."
                        )
                    elif abs(_phase) >= 3:
                        st.warning(
