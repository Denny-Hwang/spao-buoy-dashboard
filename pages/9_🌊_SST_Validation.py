                ...
                if pd.notna(_phase):
                    if abs(_phase) >= 6:
                        st.error(
                            ...
                            "the same solar cycle is the classic "
                            "signature of **the spreadsheet's Display "
                            "Time Zone being non-UTC**: Google Sheets "
                            "reformats `new Date().toISOString()` into "
                            "the sheet's local zone (e.g. Asia/Seoul), "
                            "gspread then reads back a naive local "
                            "string, and downstream code assumes UTC — "
                            "so every ERA5 / satellite lookup uses the "
                            "wrong hour.\n\n"
                            "**Fix:** set `SHEETS_DISPLAY_TZ` (Streamlit "
                            "secret **and** GitHub Actions secret) to "
                            "the spreadsheet's zone — e.g. "
                            "`Asia/Seoul`. Then clear the `ENRICH_FLAG` "
                            "column (or re-run the backfill with those "
                            "rows forced) so the affected enriched "
                            "cells are recomputed against the correct "
                            "UTC hour."
                        )
                    elif abs(_phase) >= 3:
                        st.warning(
