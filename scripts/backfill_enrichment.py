...
def _sheets_display_tz() -> str:
    """Return the spreadsheet's Display Time Zone from the cron env.

    Mirrors :func:`utils.sheets_client._get_sheets_display_tz`. Defaults
    to ``"UTC"`` for sheets that already store in UTC. A sheet is always
    stored in a single zone, so this is column-uniform — see the
    commentary in sheets_client for why per-row GPS-based lookup was
    tried and reverted (outlier GPS fixes breaking chronological
    ordering in the Data Explorer).
    """
...
def _coerce_ts(
    ...
    *,
    ...
    # lat/lon accepted for API compatibility only; ignored.
    lat: Any = None,
    lon: Any = None,
) -> pd.Timestamp:
    ...
    Column-uniform: all rows get the same zone treatment so a single
    outlier GPS fix cannot shift one row's hour bucket relative to
    its neighbours.
    """
    if pd.isna(val):
        return pd.NaT
    ...
    resolved = src_tz or _sheets_display_tz()
    try:
...
def filter_by_window(
    ...
    if start is None and end is None:
        return df.index
    # Localize naive timestamps with the sheet-wide SHEETS_DISPLAY_TZ
    # before comparing against UTC window bounds.
